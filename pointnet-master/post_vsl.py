...
def main():
	...
	loss_data = io.loadmat('data/airplane_vox_split.mat')
  train_target0 = loss_data['train']
  train_index = np.arange(626)
  del train_index[50]
  train_target = train_target[train_index,:]
  test_target = loss_data['test']
	...
	for epoch in range(total_epoch):
	    ...
	    for i in range(train_batch):
	        x_train = train_all[batch_size*i:batch_size*(i+1),:]
	        x_train = pointnet_pre_func(x_train) #Pointnet ready
	        loss = train_target[batch_size*i:batch_size(i+1),:]
